...
def objective_function(X, y, w, lamb):
    ...
    regularization = (.5 * lamb * np.linalg.norm(w)) ** 2
    # compute second term
    hinge_los = np.maximum(0, 1 - y * X.dot(w.squeeze())).sum() / len(X)
    # compute loss by summing computed terms
    obj_value = regularization + hinge_los
...
def pegasos_train(Xtrain, ytrain, w, lamb, k, max_iterations):
    ...
    for iter in range(1, max_iterations + 1):
        ...
        # Step 3
        X, y = Xtrain[A_t], ytrain[A_t]
        m = y * X.dot(w.squeeze()) < 1
        X, y = X[m], y[m]

        #if not X.size:
        #    train_obj.append(train_obj[-1])  # replicating previous step's loss for consistency
        #    continue
...
def pegasos_test(Xtest, ytest, w_l):
    ...
    """
    # you need to fill in your solution here
    ypred = np.where(np.array(Xtest).dot(w_l.squeeze()) < 0, -1, 1)
    # compute accuracy as per formula specified in Q1.3
    test_acc = (ypred == np.array(ytest)).sum() / len(Xtest)
