...
def regularized_linear_regression(X, y, lambd):
    ...
    (num_samples, D) = np.shape(X)
    x_t = np.transpose(X)
    lambd_t = lambd*(np.identity(D))
    x_t_x_inv = np.linalg.inv(np.dot(x_t,X)+lambd_t)
    w = np.dot(np.dot(x_t_x_inv,x_t),y)
    return w
...
def tune_lambda(Xtrain, ytrain, Xval, yval, lambds):
    ...
    bestlambda = 0
    for lam in lambds:
        err = test_error(regularized_linear_regression(Xtrain,ytrain,lam),Xval,yval)
        if err < best_err:
            bestlambda = lam
            best_err = err
    return bestlambda
...
def test_error(w, X, y):
    """
      ...
      """
    err = ((np.dot(X,w) - y)**2).mean(axis=0)
    return err
