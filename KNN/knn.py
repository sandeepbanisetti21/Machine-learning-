...
def compute_distances(Xtrain, X):
    ...
    (num_train, D) = np.shape(Xtrain)
    (num_test, D) = np.shape(X)
    dists = np.zeros(shape=(num_test, num_train))
...
def predict_labels(k, ytrain, dists):
    ...
    #				 YOUR CODE HERE					                    #
    #####################################################
    (num_test, num_train) = np.shape(dists)
    ypred = np.zeros(shape=(num_test))
    for i in range(0, num_test):
        (values, counts) = np.unique(ytrain[np.argpartition(dists[i], k)[:k]], return_counts=True)
...
def compute_accuracy(y, ypred):
    ...
    num_test = np.shape(y)
    counter = 0
    for x in range(0, num_test[0]):
        if y[x] == ypred[x]:
            counter += 1
    acc = counter / num_test[0]
    return acc
...
def find_best_k(K, ytrain, dists, yval):
    ...
    #				 YOUR CODE HERE					                    #
    #####################################################
    validation_accuracy = [];best_validation = float('-inf');best_k = 0
    for k in K:
        acc = compute_accuracy(yval, predict_labels(k, ytrain, dists))
