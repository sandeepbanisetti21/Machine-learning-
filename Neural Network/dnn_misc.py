...
class linear_layer:
    ...
    def backward(self, X, grad):
        ...
        np.transpose(X).dot(grad, out=self.gradient['W'])
        np.sum(grad, axis=0, keepdims=True, out=self.gradient['b'])
        self.gradient['b'] /= len(grad)
...
class relu:
    ...
    def forward(self, X):
        ...
        self.mask = X > 0  # X > 0 is the step function
