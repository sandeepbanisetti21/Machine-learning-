...
def binary_train(X, y, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    updated_w = np.insert(w, 0, b)

    updated_x_transpose = np.transpose(updated_x)
    iteration = 0
    ...
    while iteration < max_iterations:
        calculated_sigmoid = vectorized_sigmoid(updated_x.dot(updated_w))

        updated_w = updated_w + (1 / N) * (step_size * (updated_x_transpose.dot(y)) - step_size * (
            updated_x_transpose.dot(calculated_sigmoid)))
...
def binary_predict(X, w, b):
    ...
    preds = vectorized_sigmoid(X.dot(w) + b)
...
def multinomial_train(X, y, C,
    ...
    vectorized_exponential = np.vectorize(np.exp)
    updated_x = np.insert(X, 0, 1, axis=1)
    updated_w = np.insert(w, 0, b, axis=1)

    iteration = 0
    updated_y = np.zeros((C, N))
    for index, value in enumerate(y):
        updated_y[value][index] = 1

    while iteration < max_iterations:
        weightdotX = updated_w.dot(np.transpose(updated_x))
        normalized_product = weightdotX - weightdotX.max(axis=0)
        exponential_product = vectorized_exponential(normalized_product)
        probabilities = exponential_product / exponential_product.sum(axis=0)
        finalProbabilities = probabilities - updated_y
        updated_w = updated_w - (1 / N) * (step_size * (finalProbabilities.dot(updated_x)))
        iteration += 1

    b = updated_w[:, 0]
    w = np.delete(updated_w, 0, 1)
...
def multinomial_predict(X, w, b):
    ...
    classWeights = X.dot(weight_transpose) + np.transpose(b)

    sigmoid_val = vectorized_exp(classWeights)
    preds = np.argmax(sigmoid_val, axis=1)
...
def OVR_train(X, y, C, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    vectorized_y_tilda = np.vectorize(y_tilda)

    for label in range(0, C):
        updated_y = vectorized_y_tilda(y, label)
        weights, bias = binary_train(X, updated_y, step_size=step_size, max_iterations=max_iterations)
        w[label] = weights
        b[label] = bias
...
def OVR_predict(X, w, b):
    ...
    sigmoid_val = vectorized_sigmoid(classWeights)
    preds = np.argmax(sigmoid_val, axis=1)
